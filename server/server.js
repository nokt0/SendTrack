...
function requestToken () {
  ...
  const content = fs.readFileSync('./private/token.json', 'utf8')
  ...
  } else { return { token: token.key, expires: token.expires } }
};
...
app.get('/search', async (request, response) => {
  ...
  const spotify = requestData.spotify(request.query, constants.SEARCH_REQUEST)
  const youtube = requestData.youtube(request.query, constants.SEARCH_REQUEST)
  const deezer = requestData.deezer(request.query, constants.SEARCH_REQUEST)
  const promisesArr = [spotify, youtube, deezer]
...
app.get('/track', (request, response) => {
  const responseJson = { ...constants.RESPONSE_JSON }
})
...
deezerRouter.get('/track', function (request, response) {
  ...
  const requestOptions = deezerOptions
...
deezerRouter.get('/search', function (request, response) {
  ...
  if (track && artist) { requestOptions.url += 'track:"' + track + '" artist:"' + artist + '"' } else if (artist) { requestOptions.url += 'artist:"' + artist + '"' } else if (track) { requestOptions.url += track }
...
app.use('/SendTrack', express.static(path.join(__dirname, '/build')))
app.use('/SendTrack/static', express.static(path.join(__dirname, '/build/static')))
