...
export function searchByWord (str) {
  if (str === '') { return { notValid: 'empty string' } }
  let splitedString
  if (str.indexOf('-') !== -1) { splitedString = str.split('-') }
  if (str.indexOf('–') !== -1) { splitedString = str.split('–') }
  ...
  if (splitedString.length > 1) { objectToCompare = createObjectToCompare(splitedString[0].trim(), splitedString[1].trim()) }
...
export function urlWorker (url) {
  ...
  if (serviceObj.service === ServicesUrl.youtube || serviceObj.service === ServicesUrl.youtubeMusic) {
    const requestObj = requestYoutubeObject(
    ...
    let resUrl
    if (serviceObj.service === 'youtube') { resUrl = 'https://www.youtube.com/watch?v=' + requestObj.items[0].id } else if (serviceObj.service === 'youtubeMusic') { resUrl = 'https://www.music.youtube.com/watch?v=' + requestObj.items[0].id }

    objectToCompare = createObjectToCompare(artist, track, resUrl, serviceObj.service)

    index = objectToCompare.artist.indexOf(' - Topic')
    if (index !== -1) { objectToCompare.artist = objectToCompare.artist.substring(0, index) }
  ...
  if (serviceObj.service === ServicesUrl.spotify) {
    const requestObj = requestSpotifyObject(
    ...
    const url = requestObj.external_urls.spotify
  ...
  if (serviceObj.service === ServicesUrl.deezer) {
    const requestObj = requestDeezerObject(createDeezerArguments(serviceObj), 'track')
...
export function urlValidator (url) {
  let index
  if (url.indexOf('.') !== -1) { index = url.indexOf('.') } else { return false }
...
function requestYoutubeObject (argumentsObj, requestType) {
  ...
  if (xhr.status !== 200) {
    ...
    if (JSON.parse(xhr.responseText).pageInfo.totalResults !== 0) { videoInfoObj = JSON.parse(xhr.responseText) } else { videoInfoObj.notValid = 'Not Found' }
  }
...
function requestSpotifyObject (argumentsObj, requestType) {
  ...
  if (xhr.status !== 200) {
    ...
    // вывести результат
    // responseText -- текст ответа.
    if (JSON.parse(xhr.responseText).total !== 0) { requestObj = JSON.parse(xhr.responseText) }
  }
...
function createDeezerArguments (serviceObj) {
  ...
  const arrayArguments = cutUrl.split('/')
  if (arrayArguments[0] === 'artist' || arrayArguments[0] === 'track') { argumentsObj[arrayArguments[0]] = arrayArguments[1] }
...
export function createArrayOfUrls (objectToCompare) {
  ...
  if (objectToCompare.initialService !== ServicesUrl.spotify) {
    ...
    const requestedObj = requestSpotifyObject(spotifyRequest, 'search')
    ...
    if (!requestedObj.hasOwnProperty('type') && requestedObj.type !== 'album' &&
      ...
      const returnedItem = searchInSpotifyObject(requestedObj, objectToCompare)
      if (!returnedItem.hasOwnProperty('notValid')) {
        let artists = ''
        for (var i = 0; i < returnedItem.artists.length; i++) {
          if (i !== returnedItem.artists.length - 1) { artists += returnedItem.artists[i].name + ', ' } else { artists += returnedItem.artists[i].name }
        }
  ...
  if (objectToCompare.initialService !== ServicesUrl.youtubeMusic || objectToCompare.initialService !== ServicesUrl.youtube) {
    ...
    const requestedObj = requestYoutubeObject(youtubeRequest, 'search')
    if (!requestedObj.hasOwnProperty('notValid')) {
      const returnedItem = searchInYoutubeObject(requestedObj, objectToCompare)
      if (!returnedItem.hasOwnProperty('notValid')) {
        if (returnedItem.snippet.channelTitle.indexOf('- Topic') === -1) {
          ...
          if (objectToCompare.initialService !== ServicesUrl.youtubeMusic) { arrayOfUrls.youtubeMusic = writeProps(artist, title, 'https://music.youtube.com/watch?v=' + returnedItem.id.videoId, returnedItem.snippet.thumbnails.high.url) }
          if (objectToCompare.initialService !== ServicesUrl.youtube) { arrayOfUrls.youtube = writeProps(artist, title, 'https://youtube.com/watch?v=' + returnedItem.id.videoId, returnedItem.snippet.thumbnails.high.url) }
        } else {
  ...
  if (objectToCompare.initialService !== ServicesUrl.deezer) {
    ...
    const requestedObj = requestDeezerObject(deezerRequest, 'search')
    if (!requestedObj.hasOwnProperty('notValid')) {
      const item = searchInDeezerObject(requestedObj, objectToCompare)
...
function searchInYoutubeObject (youtubeReturnedObject, objectToCompare) {
  ...
  for (const item of youtubeReturnedObject.items) {
    if (item.snippet.channelTitle.indexOf('- Topic') !== -1) {
      ...
      if (!matchStringsWithoutSpecs(trackName, item.snippet.title) && !matchStringsWithoutSpecs(item.snippet.title, trackName)) { continue }
      if (!matchStringsWithoutSpecs(artistName, validTitle) && !matchStringsWithoutSpecs(validTitle, artistName)) { continue }
      similarObject = item
      foundFlag = true
      break
    }
  ...
  if (!foundFlag) {
    for (const item of youtubeReturnedObject.items) {
      if (!matchStringsWithoutSpecs(artistName + ' ' + trackName, item.snippet.title) && !matchStringsWithoutSpecs(item.snippet.title, artistName + ' ' + trackName)) { continue }
      similarObject = item
      break
    }
  }
...
function searchInSpotifyObject (spotifyReturnedObject, objectToCompare) {
  ...
  for (const item of spotifyReturnedObject.tracks.items) {
    ...
    if (item.type === 'track') {
      if (!matchStringsWithoutSpecs(trackName, item.name) && !matchStringsWithoutSpecs(item.name, trackName)) { continue }
      ...
      if (validator) { return similarObject }
    }
  }
...
function searchInDeezerObject (deezerReturnedObject, objectToCompare) {
  ...
  for (var item of deezerReturnedObject.data) {
    if (item.type === 'track') {
      if (!matchStringsWithoutSpecs(trackName, item.title) && !matchStringsWithoutSpecs(item.title, trackName)) { continue }
...
function checkService (inputUrl) {
  ...
  for (const Service in ServicesUrl) {
    const found = checkServiceUrl(url, ServicesUrl[Service])
    if (found !== -1) {
      index = found
...
function checkServiceUrl (urlString, serviceString) {
  ...
  if (index !== -1) { index += serviceString.length }
...
function checkTokenInCookie () {
  ...
  if (buffer) { return buffer } else { return getSpotifyToken() }
}

export function getLog () {
  let log = ''
  for (const elem of logArray) {
    log += elem
  }
  return log
}
